...
/**
 * Singleton class for HDFC Payment Handler
 */
class PaymentHandler {
  ...
  constructor(userSpecifiedConfigPath) {
    ...
    const configPath = userSpecifiedConfigPath || path.join(__dirname, "config.json");
    try {
      const config = fs.readFileSync(configPath, "utf-8");
      this.paymentConfigs = JSON.parse(config);
    } catch (error) {
      console.error(
        "Failed to read configs from file, here's the error message:- " +
          error.message
      );
  ...
  makeServiceCall({ apiTag, path, method, headers = {}, query = {}, body }) {
    return new Promise((resolve, reject) => {
      ...
      req.setTimeout(100000, () => {
        this.logger.error(apiTag, paymentRequestId, "Request has been timedout", "");
        req.destroy(
          new APIException(
            -1,
            "REQUEST_TIMEOUT",
